...
- Legacy localStorage message items and legacy localStorage memory items are normalized on load with the same role/text validation, trimming, and blank-record filtering used for JSON imports, then written back when normalization changes the stored content.
- Scam false-positive tuning now avoids a shopping false positive from bare `pin` matching, avoids flagging a stranger asking for directions, and avoids flagging routine government or benefits-office visits without payment/code pressure.
