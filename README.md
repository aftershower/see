...
The browser app sends chats through `/api/chat` as a stable boundary for future AI providers. It sends only a minimized non-sensitive memory context with type and label, not full memory detail. The endpoint also enforces that minimization server-side by accepting only valid non-sensitive type/label memory items, capped at 8. In V1 the endpoint uses the built-in demo companion engine, and the browser falls back to the same local engine if the request times out or fails.
