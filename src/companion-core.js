...
export function generateCompanionReply({ text = "", memories = [], now = new Date(), locale = "zh-CN" } = {}) {
  ...
  if (safety.level === "scam") {
    return {
      text: "这可能是诈骗。先慢下来，别转钱，也别把钱转到所谓安全账户来保护资金。别通过 Zelle、Venmo、Cash App 或其他支付 App 付款，也别给验证码、礼品卡号码或 PIN。请保存收据和聊天记录，用你自己找来的官方电话核实，也可以先问一个信得过的人。美国可到 ReportFraud.ftc.gov 或 IC3.gov 报告。",
      safety,
      memories: []
    };
  }
