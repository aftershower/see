...
const SCAM_PATTERN = /转账|礼品卡|验证码|六位数|短信码|动态码|一次性密码|银行卡|陌生人|中奖|汇款|(?:社保局|社安局|税务局|政府).{0,16}(?:转账|汇款|验证码|礼品卡|银行卡|密码|下载|屏幕共享|保密|钱|现金)|(?:apple|google play|itunes|steam|target|walmart|amazon).{0,12}(?:礼品卡|gift card)|\bpin\b|密码|银行密码|比特币|加密货币|礼品卡号码|远程控制|远程操作|电脑客服|技术支持|快递.*取.*现金|取现金|别告诉家人|不要告诉任何人|保密|自称(?:警察|公安|客服|社保局|社安局|税务局|政府)|下载.*(?:app|软件)|屏幕共享|共享屏幕|骗子|(?:不确定|陌生人|骗子|自称|让我|要求).{0,12}链接|链接.{0,12}(?:骗子|陌生人|转账|验证码|下载|安全)|(?:陌生人|骗子|自称|让我|要求).{0,12}身份证|身份证.{0,12}(?:照片|号码|发给|转账|验证码)|gift card|wire transfer|verification code|otp|one-time password|passcode|qr code|refund|crypto|bitcoin|remote access|tech support|cash pickup|courier/i;
