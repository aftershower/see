...
test("optimization notes track family emergency scam tuning", async () => {
  const notes = await readFile(new URL("../docs/research/2026-06-02-optimization-research.md", import.meta.url), "utf8");

  assert.match(notes, /family emergency/i);
  assert.match(notes, /grandparent scam/i);
  assert.match(notes, /bail money/i);
  assert.match(notes, /FTC/i);
  assert.match(notes, /ReportFraud\.ftc\.gov/i);
});
...
test("optimization notes track contextual scam false-positive tuning", async () => {
  const notes = await readFile(new URL("../docs/research/2026-06-02-optimization-research.md", import.meta.url), "utf8");

  assert.match(notes, /refund false positive/i);
  assert.match(notes, /bitcoin|crypto/i);
  assert.match(notes, /QR code/i);
  assert.match(notes, /tech support/i);
  assert.match(notes, /FTC/i);
});

test("optimization notes track everyday payment false-positive tuning", async () => {
  const notes = await readFile(new URL("../docs/research/2026-06-02-optimization-research.md", import.meta.url), "utf8");

  assert.match(notes, /gift card birthday false positive/i);
  assert.match(notes, /verification code self-login/i);
  assert.match(notes, /routine transfer|daily transfer/i);
  assert.match(notes, /courier delivery/i);
  assert.match(notes, /FTC/i);
});
...
test("optimization notes track bank app false-positive tuning", async () => {
  const notes = await readFile(new URL("../docs/research/2026-06-02-optimization-research.md", import.meta.url), "utf8");

  assert.match(notes, /bank app false positive/i);
  assert.match(notes, /funds safety/i);
  assert.match(notes, /transfer limit/i);
  assert.match(notes, /bank app download/i);
